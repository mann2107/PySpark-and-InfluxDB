...
def create_json(value):
    ...
    parent = {"measurement": "temperature-counter",
              "tags": {
                  "host": "server01",
                  "region": "us-west",
                  "source": "BDS"
...
if __name__ == '__main__':
    ...
    rs = client.query('select * from temperature limit 50000')
    data = sc.parallelize(rs)
    name_value = data.flatMap(lambda x: x).map(lambda x: (x['point_name'], str(x['temp']) + '|' + x['time']))
